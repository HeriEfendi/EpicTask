  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
    <div class="glass-modal w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-2xl animate-slide-up">
      ...
      <div class="text-center mb-6">
        <div class="inline-flex p-3 rounded-2xl bg-gradient-to-tr from-blue-600 to-indigo-600 shadow-md shadow-blue-500/20 mb-3">
          <img src="/src/assets/logo.svg" alt="EpicTask" class="w-8 h-8" />
        </div>
        <h1 class="text-xl font-black tracking-tight text-slate-900">EpicTask</h1>
        <p class="text-xs text-slate-500 mt-1">Next-Gen Project & Issue Tracking with Real-Time Boards</p>
      </div>

      <!-- Quick 1-Click Demo Logins -->
      <div class="mb-6 p-3.5 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl space-y-2">
        <div class="flex items-center space-x-1.5 text-xs font-bold text-blue-800">
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          <button
            ...
            class="p-2 bg-white hover:bg-blue-50/80 border border-blue-100 rounded-lg text-left transition shadow-2xs"
          >
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4 text-xs">
        <div v-if="isRegister">
          ...
          <div class="relative">
            ...
            <input
              ...
              class="w-full bg-slate-50 border border-slate-300 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 focus:outline-none focus:border-blue-500 focus:bg-white"
              required
            />
          </div>
        </div>
        ...
        <button
          type="submit"
          :disabled="loading"
          class="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold text-xs shadow-md shadow-blue-600/25 flex items-center justify-center space-x-1.5 transition active:scale-95 disabled:opacity-50"
        >
